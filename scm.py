...
class ppo():
    ...
    def make_policy(self):
        ...
        policy.compile(loss=proximal_policy_optimization_loss(advantage=advantage,action=action), optimizer=tf.keras.optimizers.legacy.Adam(lr=0.0001))
        return policy
    ...
    def make_critic(self):
        ...
        critic.compile(loss="mse", optimizer=tf.keras.optimizers.legacy.Adam(lr=0.0001), metrics=['accuracy'])
        return critic
    ...
    def adv_calc(self,val,val_,rew,done):
        ...
        return returns
    def train(self,batch=64,epochs=10):
...
#Calling Demand and Lead Time from function instead of training the model on a fixed demand and lead time.
def getDemandfromFn(min, max):
    demand = np.round(np.random.uniform(min, max, 35)).tolist()
    return demand

def getLeadTimefromFn(min, max):
    lead_time = np.round(np.random.uniform(min, max, 35)).tolist()
    return lead_time
...
for ep in range(0,episodes):
    ...
    state = env.reset(getDemandfromFn(0, 15), getLeadTimefromFn(0, 4)) #for each episodes, demand and lead time will be different
    s= std_scale(state)
    st_buffer.reset()
    st_buffer.append(s)
    ...
    for step in range(steps):
        ...
        reqs = np.array([state[0][4],state[1][4],state[2][4],state[3][4]])
        ...
        agent.memory.store(st_b.ravel(),action,st_b_.ravel(),r,done,out,cov_pred)
